"""WebSocket endpoints for CosyVoice2 - Real-time bidirectional streaming"""
import asyncio
import json
import logging
import time
...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, client_id: str = None) -> str:
        ...
        logger.info(f"WebSocket connected: session_id={session_id}, client_id={client_id}")
        return session_id
...
async def websocket_streaming_endpoint(
    ...
    - Client sends text synthesis requests
    - Server streams back audio chunks in real-time
    - Supports multiple concurrent synthesis requests
    """
    ...
    try:
        ...
        session_id = await ws_manager.connect(websocket, client_id)
        
        # Send connection confirmation
        await ws_manager.send_message(session_id, WebSocketStatus(
            timestamp=time.time(),
            status="connected",
            message=f"WebSocket session {session_id} established"
        ))
...
async def handle_synthesis_request(
    session_id: str, 
    message_data: dict, 
    streaming_engine: StreamingSynthesisEngine,
    request_id: str = None
):
    """Handle synthesis request from WebSocket client"""
    
    try:
        # Parse synthesis request
        synthesis_request = WebSocketSynthesisRequest(**message_data)
        
        # Send synthesis start confirmation
        await ws_manager.send_message(session_id, WebSocketSynthesisStart(
            request_id=request_id,
            timestamp=time.time(),
        ...
        async for chunk_bytes, metadata in streaming_engine.stream_cross_lingual_synthesis(streaming_request):
            ...
            await ws_manager.send_message(session_id, audio_chunk)
            chunk_count += 1
            
            # Check if this is the final chunk
            if metadata.is_final:
                total_duration = chunk_count * (metadata.sample_rate / 1000.0)  # Rough estimate
                break
        
        # Send synthesis complete confirmation
        synthesis_time = time.time() - start_time
        await ws_manager.send_message(session_id, WebSocketSynthesisComplete(
            request_id=request_id,
            timestamp=time.time(),
        ...
        logger.info(f"WebSocket synthesis completed: {chunk_count} chunks, {synthesis_time:.2f}s")
