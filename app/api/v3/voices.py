"""
Voice management API endpoints - v3 (CosyVoice3)
"""
...
router = APIRouter(prefix="/voices", tags=["Voice Management (v3)"])
...
async def create_voice(
    ...
    """Create a new voice in the cache (CosyVoice3)"""
    ...
    try:
        ...
        logger.info(f"Created voice (v3): {voice_id}")
        ...
        logger.error(f"Error creating voice (v3) {voice_id}: {e}")
...
async def list_voices(
    ...
    """List all voices with optional filtering and pagination (CosyVoice3)"""
    ...
    try:
        ...
        logger.error(f"Error listing voices (v3): {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_voice(
    ...
    """Get a specific voice by ID (CosyVoice3)"""
    ...
    try:
        ...
        logger.error(f"Error getting voice (v3) {voice_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def update_voice(
    ...
    """Update a voice's information (CosyVoice3)"""
    ...
    try:
        ...
        logger.info(f"Updated voice (v3): {voice_id}")
        return VoiceResponse(**voice.dict())

    except VoiceNotFoundError:
        raise HTTPException(status_code=404, detail=f"Voice '{voice_id}' not found")
    except Exception as e:
        logger.error(f"Error updating voice (v3) {voice_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def delete_voice(
    ...
    """Delete a voice from the cache (CosyVoice3)"""
    ...
    try:
        ...
        logger.info(f"Deleted voice (v3): {voice_id}")
        return {"message": f"Voice '{voice_id}' deleted successfully"}

    except VoiceNotFoundError:
        raise HTTPException(status_code=404, detail=f"Voice '{voice_id}' not found")
    except Exception as e:
        logger.error(f"Error deleting voice (v3) {voice_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_voice_stats(
    ...
    """Get voice cache statistics (CosyVoice3)"""
    ...
    try:
        ...
        logger.error(f"Error getting voice stats (v3): {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def list_pretrained_voices(
    ...
    """Get list of available pre-trained voices (CosyVoice3)"""
    ...
    try:
        ...
        logger.error(f"Error listing pre-trained voices (v3): {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
