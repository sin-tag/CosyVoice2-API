"""Streaming API endpoints for CosyVoice2 - v2 Real-time audio streaming"""
import logging
import asyncio
import time
import json
...
async def stream_cross_lingual_synthesis(
    ...
    """Stream cross-lingual synthesis with real-time audio chunks (CosyVoice2)"""

    logger.info(f"Starting streaming synthesis (v2): text='{text[:50]}...', voice_id='{voice_id}'")
...
async def stream_cross_lingual_chunked(
    ...
    """Stream cross-lingual synthesis with chunked transfer encoding (CosyVoice2)"""

    logger.info(f"Starting chunked streaming (v2): text='{text[:50]}...', voice_id='{voice_id}'")
...
async def stream_cross_lingual_sse(
    ...
    """Stream cross-lingual synthesis using Server-Sent Events (CosyVoice2)"""

    logger.info(f"Starting SSE streaming (v2): text='{text[:50]}...', voice_id='{voice_id}'")
...
async def streaming_health_check(
    ...
    """Health check for streaming functionality (v2)"""
    try:
        return {
            "status": "healthy",
            "version": "v2",
