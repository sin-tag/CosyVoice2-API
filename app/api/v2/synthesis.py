"""
Cross-lingual synthesis API endpoints - v2 (CosyVoice2)
"""
...
router = APIRouter(prefix="/cross-lingual", tags=["Cross-lingual Voice Cloning (v2)"])
...
async def cross_lingual_with_audio(
    ...
    """Cross-lingual voice cloning with audio file (CosyVoice2)"""
    ...
    try:
        ...
        try:
            ...
            # Update URL to v2 path
            result.audio_url = result.audio_url.replace("/api/v1/", "/api/v2/")
            logger.info(f"Cross-lingual synthesis (v2) with audio completed")
            return result
...
async def cross_lingual_with_cache(
    ...
    """Cross-lingual voice cloning with cached voice (CosyVoice2)"""

    try:
        result = await synthesis_engine.synthesize_cross_lingual_with_cache(request)
        # Update URL to v2 path
        result.audio_url = result.audio_url.replace("/api/v1/", "/api/v2/")
        logger.info(f"Cross-lingual synthesis (v2) with cache completed for voice: {request.voice_id}")
        return result
    except VoiceNotFoundError as e:
        logger.warning(f"Voice not found: {e}")
        raise HTTPException(status_code=404, detail=str(e))
...
async def get_audio_file(filename: str):
    ...
    try:
        ...
        logger.info(f"Serving audio file (v2): {filename} -> {file_path}")
...
async def create_async_synthesis_task(
    ...
    """Create async cross-lingual synthesis task (CosyVoice2)"""

    try:
        result = await async_manager.create_task(request)
        logger.info(f"Created async synthesis task (v2): {result.task_id}")
        return result
    except Exception as e:
